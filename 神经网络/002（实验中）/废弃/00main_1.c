...
int main()
{
	...
	/*
	puts("数据集整理完成");
	for (size_t i = 0; i < sizeof(训练数据)/sizeof(训练数据[0]); i++)
	{
		printf("[%.1f, %.1f]  [%.0f %.0f %.0f %.0f %.0f]\n",
			训练集.自变量数据集[2*i],
			训练集.自变量数据集[2*i + 1],
			训练集.函数值数据集[5*i],
			训练集.函数值数据集[5*i + 1],
	...
	*/
	...
	puts("创建神经网络");
	激励函数_t 各层激励函数[_层数] = {三次方激励函数, 三次方激励函数, 三次方激励函数};
	//激励函数_t 各层激励函数[_层数] = {sigmoid激励函数, sigmoid激励函数, sigmoid激励函数};
